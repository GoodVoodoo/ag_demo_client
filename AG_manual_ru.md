...
Для запуска процесса клонирования голоса, используйте команду `clone` и укажите путь к вашему аудиофайлу. Команда вернет ID задачи, который можно использовать для отслеживания прогресса.

```bash
audiogram vc clone --audio-file 1297.wav
```
...
Voice cloning task created with ID: c10n3-t4sk-1d-3x4mpl3
```

#### 2. Проверить статус задачи

Используйте команду `get-task-info` с ID задачи из предыдущего шага, чтобы проверить статус процесса клонирования.

```bash
audiogram vc get-task-info --task-id c10n3-t4sk-1d-3x4mpl3
```

Пример вывода после завершения задачи:
```
Connecting to gRPC server - grpc.audiogram-demo.mts.ai:443

Task ID: c10n3-t4sk-1d-3x4mpl3
Status: Ready
Voice ID: v01c3-1d-3x4mpl3
```

#### 3. Удалить клонированный голос

После того как голос будет клонирован, он сохранится в системе. Если он вам больше не нужен, вы можете удалить его с помощью команды `delete` и `voice_id`, полученного из информации о задаче.

```bash
audiogram vc delete --voice-id v01c3-1d-3x4mpl3
```
...
Voice with ID 'v01c3-1d-3x4mpl3' deleted successfully.
```
